...
function runChainOfThought(problem: string, maxSteps: number): ReasonResult {
  ...
  for (let i = 0; i < Math.min(maxSteps, 10); i++) {
    steps.push({
      step: i + 1,
      content: `Step ${i + 1}: Analyzing "${thought.substring(0, 100)}${thought.length > 100 ? "..." : ""}"`,
      type: "thought",
    });
  ...
  steps.push({
    step: steps.length + 1,
    content: "Conclusion: Problem requires systematic decomposition and verification at each stage.",
    type: "result",
  });

  return {
    strategy: "cot", problem, steps,
    conclusion: "Break the problem into smaller, verifiable sub-problems and solve each sequentially.",
...
function runTreeOfThoughts(problem: string): ReasonResult {
  ...
  for (let d = 0; d < depth; d++) {
    for (let b = 0; b < breadth; b++) {
      steps.push({
        step: d * breadth + b + 1,
        content: `Branch ${b + 1} at depth ${d + 1}: Exploring approach ${String.fromCharCode(65 + b)}`,
        type: "thought",
      });
    }
    steps.push({
      step: (d + 1) * breadth + 1,
      content: `Depth ${d + 1} evaluation: Branch ${Math.floor(Math.random() * breadth) + 1} shows most promise.`,
      type: "observation",
    });
  }

  return {
    strategy: "tot", problem, steps,
    conclusion: "Tree-of-Thoughts exploration complete. Most promising path identified through iterative branching.",
...
function runReAct(problem: string): ReasonResult {
  ...
  for (let i = 0; i < maxIter && i < 5; i++) {
    steps.push({ step: i * 3 + 1, content: `Thought: Need to examine aspect #${i + 1}.`, type: "thought" });
    steps.push({ step: i * 3 + 2, content: `Action: Executing investigation on "${problem.substring(0, 50)}..."`, type: "action" });
    steps.push({ step: i * 3 + 3, content: `Observation: Result indicates ${i % 2 === 0 ? "positive" : "needs adjustment"}.`, type: "observation" });
  }

  return {
    strategy: "react", problem, steps,
...
function runPlanExecute(problem: string): ReasonResult {
  const steps: ReasonStep[] = [
    { step: 1, content: "Phase 1 — PLAN: Analyzing goal and decomposing into sub-tasks.", type: "plan" },
  ...
  return {
    strategy: "plan_execute", problem, steps,
    conclusion: "Plan-and-Execute complete. All sub-tasks executed and verified in dependency order.",
...
const reasonTool: Tool = {
  definition: {
    ...
    params: [
      { name: "problem", type: "string", description: "The problem or question to reason about", required: true },
      { name: "strategy", type: "string", description: "Reasoning strategy to use", required: false },
      { name: "context", type: "string", description: "Optional context to include in reasoning", required: false },
      { name: "max_steps", type: "number", description: "Maximum reasoning steps (default: 10)", required: false },
    ],
    capabilities: [],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      switch (strategy) {
        case "tot": result = runTreeOfThoughts(fullProblem); break;
        case "react": result = runReAct(fullProblem); break;
        case "plan_execute": result = runPlanExecute(fullProblem); break;
        default: result = runChainOfThought(fullProblem, maxSteps); break;
      }
...
const listStrategies: Tool = {
  definition: {
    ...
    description: "List all available reasoning strategies with descriptions and best-use recommendations.",
    params: [],
    capabilities: [],
  },
  ...
  execute: async (_args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      const strategies = Object.values(STRATEGIES).map((s) => ({
        ...
        bestFor: s.bestFor, workflow: s.steps,
      }));

      return {
        toolName: "list_strategies", success: true,
        output: JSON.stringify({
          strategies,
          default: config.defaultStrategy,
          config: { maxTreeDepth: config.maxTreeDepth, treeBreadth: config.treeBreadth, reactMaxIterations: config.reactMaxIterations },
        }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
        toolName: "list_strategies", success: false, output: "",
...
const selectStrategy: Tool = {
  definition: {
    ...
    params: [
      { name: "task_description", type: "string", description: "Description of the task", required: true },
      { name: "task_type", type: "string", description: "Type of task", required: false, enum: ["coding", "debugging", "planning", "analysis", "creative", "math", "decision"] },
    ],
    capabilities: [],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.task_description || typeof args.task_description !== "string") {
        return { toolName, success: false, output: "", error: "task_description must be a non-empty string", durationMs: Date.now() - start };
      }
      ...
      if (descLower.includes("debug") || descLower.includes("fix") || descLower.includes("error")) {
      ...
      } else if (descLower.includes("plan") || descLower.includes("design") || descLower.includes("architect")) {
        ...
        reason = `Selected ${strategy.name} because task type "${taskType}" matches strategy strengths.`;
      } else {
      ...
      return {
        toolName, success: true,
        output: JSON.stringify({ selected: strategy.id, name: strategy.name, description: strategy.description, reason, workflow: strategy.steps }),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
        toolName, success: false, output: "",
        error: `Strategy selection failed: ${error instanceof Error ? error.message : String(error)}`,
        durationMs: Date.now() - start,
      };
    }
  },
...
const evaluateReasoning: Tool = {
  definition: {
    ...
    params: [
      { name: "reasoning_trace", type: "string", description: "The full reasoning trace to evaluate", required: true },
      { name: "criteria", type: "string", description: "Comma-separated evaluation criteria", required: false },
    ],
    capabilities: [],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.reasoning_trace || typeof args.reasoning_trace !== "string") {
        return { toolName, success: false, output: "", error: "reasoning_trace must be a non-empty string", durationMs: Date.now() - start };
      }
      ...
      for (const criterion of criteria) {
        ...
        switch (criterion) {
          case "logic":
            feedback = score > 0.7 ? "Logical flow is consistent." : "Some logical leaps detected. Add intermediate steps.";
            break;
          case "completeness":
            feedback = score > 0.7 ? "Covers key aspects of the problem." : "Some aspects not addressed. Expand coverage.";
            break;
          case "clarity":
            feedback = score > 0.7 ? "Reasoning is clear and well-articulated." : "Some steps are ambiguous.";
            break;
          default:
      ...
      const overallScore = Object.values(scores).reduce((sum, s) => sum + s.score, 0) / Object.values(scores).length;
      ...
      return {
        ...
        output: JSON.stringify({
          ...
          recommendations: overallScore < 0.7
            ? ["Add more intermediate reasoning steps", "Support claims with evidence", "Check for logical gaps"]
...
export async function onLoad(ctx: PluginContext): Promise<void> {
  ...
  ctx.logger.info("[cortex-plugin-chain-of-thought] Loaded with 4 strategies: cot, tot, react, plan_execute");
}
